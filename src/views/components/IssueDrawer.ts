...
export class IssueDrawer extends Component {
  ...
  constructor(
    ...
    header.createDiv({ cls: "issue-drawer-title-area" })
          .createEl("h2", { text: "Task Detail", cls: "issue-drawer-title" });
  ...
  private renderInfo(): void {
    ...
    const { key, title, filePath, line } = this.currentTarget;
    ...
    headerRow.createEl("h3", { text: "Notes" });
    ...
    textarea.placeholder = "Write details, comments, or instructions here…\n(Markdown supported)";

    // Auto-save
    let timeout: NodeJS.Timeout;
    textarea.addEventListener("input", () => {
      clearTimeout(timeout);
      timeout = setTimeout(() => this.issueManager.saveIssue(key, textarea.value), 500);
    });

    // Tab switching
    writeBtn.addEventListener("click", () => {
      writeBtn.addClass("is-active"); previewBtn.removeClass("is-active");
      textarea.style.display = "block"; previewArea.style.display = "none";
    });
    previewBtn.addEventListener("click", async () => {
      previewBtn.addClass("is-active"); writeBtn.removeClass("is-active");
      textarea.style.display = "none"; previewArea.style.display = "block";
      previewArea.empty();
